...
class VgDatasetText(Dataset):
    def __init__(self, vg_path, split, transforms, num_objects, vg_loss_lambda, negatives = False, sg_negatives = False, relations = 0, no_dense = 0, no_relation = False, size_ablation = 1.0):
        if no_dense > 0:
        ...
        self.sg_negatives = sg_negatives
    ...
    def __getitem__(self, idx):
        ...
        if not self.only_text:
            ...
            if self.sg_negatives:
                neg_object_descriptions = [apply_negative_object_desc(obj,self.attributes_annotations) for obj in objects]
                neg_object_descriptions = [repair_text(desc) if desc != "" else "" for desc in neg_object_descriptions]
                if missing_objects > 0:
                    neg_object_descriptions += ["" for i in range(missing_objects)]
            ...
            if self.relations:
                ...
                if self.sg_negatives:
                    neg_relation_descriptions = []
                    for relation in relations:
                        subject_id = str(relation["subject_id"])
                        object_id = str(relation["object_id"])
                        subject_description =  all_objects_dict[subject_id]["names"][0] 
                        obj_description =  all_objects_dict[object_id]["names"][0]
                        predicate = apply_negative_relation_desc(relation, self.relations_annotations)
                        if predicate == "":
                           neg_relation_descriptions.append("")
                        else:
                            neg_relation_description = subject_description + " " + predicate + " " + obj_description
                            neg_relation_descriptions.append(neg_relation_description)
                    neg_relation_descriptions = [repair_text(desc) if desc != "" else "" for desc in neg_relation_descriptions]
                    if missing_relations > 0:
                        neg_relation_descriptions += ["" for i in range(missing_relations)]
        ...
        if self.split == "train" and self.negatives:
            if not self.relations:
                if not self.sg_negatives:
                    return image, text, valid_objects, bounding_boxes, object_descriptions, neg_text, neg_mask, idx
                else:
                    return image, text, valid_objects, bounding_boxes, object_descriptions, neg_object_descriptions, neg_text, neg_mask, idx
            else:
                if not self.sg_negatives:
                    ...
                    return image, text, valid_objects, bounding_boxes, object_descriptions, neg_object_descriptions, valid_relations, relations_bounding_boxes, relations_descriptions, neg_relation_descriptions, neg_text, neg_mask, idx
