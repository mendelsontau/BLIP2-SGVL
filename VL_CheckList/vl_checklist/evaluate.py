from VL_CheckList.vl_checklist.utils import chunks, add_caption
from VL_CheckList.vl_checklist.data_loader import DataLoader
...
import json
import logging
from lavis.common import dist_utils as utils



class Evaluate(object):
    def __init__(self, config_file, model,epoch,args) -> None:    
    ...
    def eval(self, data_type):
        ...
        if self.task == 'itm':
            ...
            for name in d.data:
                ...
                starttime = time.time()
                for batch in chunks(d.data[name], self.batch_size):
                    images = [z["path"] for z in batch]
                    ...
                    for i in range(len(result_tmp)):
                        ...
                        if result_tmp[i][0] > result_tmp[i][1]:
                            sample_true.append({"img_path":images[i],"pos_score":float(round(result_tmp[i][0],4)),
                            "pos_txt":texts_pos[i],"neg_score":float(round(result_tmp[i][1],4)),
                            "neg_txt":texts_neg[i],"result":"correct"})
                            num_t += 1
                        else:
                            sample_false.append({"img_path":images[i],"pos_score":float(round(result_tmp[i][0],4)),
                            "pos_txt":texts_pos[i],"neg_score":float(round(result_tmp[i][1],4)),
