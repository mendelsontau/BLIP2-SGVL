...
class CsvDataset(Dataset):
    def __init__(self, input_filename, transforms, train_num_samples, img_key="filepath", caption_key="title", sep="\t"):
    ...
    def __getitem__(self, idx):
        while True:
            try:
                images = self.transforms(Image.open(os.path.join("/home/gamir/datasets/CC3M",str(self.images[idx]))))
                texts = str(self.captions[idx])
                break
            except:
                idx += 1
        return images, texts, idx + 1000000
...
def get_data_cc3m(args, epoch=0):
    data = {}
